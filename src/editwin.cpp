...
bool EditWidget::darkMode() const { return editor->darkMode(); }
...
QSize EditWidget::textSize() const {
  int w =
      floor((width() - leftMargin() - 2 * EditWidget::BORDER) / editor->fw());
  int h =
      floor((height() - EditWidget::BORDER - editor->fa()) / editor->fh() + 1);
  if (w < 0) // not initialized?
    w = 1;
  if (h < 0)
    h = 1;
  return QSize(w, h);
...
Pos EditWidget::screenPosToFilePos(Pos screenPos) {
  ...
  return Pos(screenPos.col + k->screenColumnOffset(),
             screenRowToFileRow(screenPos.row));
      }
...
void EditWidget::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (p.x() >= 0 && p.x() < leftMargin()) {
    Kontext *k = editor->kontext();
    int row =
        screenRowToFileRow(int((p.y() - EditWidget::BORDER) / editor->fh()));
...
void EditWidget::paintLine(DrawingContext &dc, int fileRow, int y) {
  ...
  if (fileRow >= start.row && fileRow <= end.row) {
    ...
    case SelectionMode::CharSelect:
      if (fileRow == start.row && fileRow == end.row)
        ...
        r = QRect(xToPixel(start.col), yy,
                  (dc.visibleColumns - start.col) * dc.fw, dc.fh + 2);
  ...
  if (l.label() != ' ') {
    ...
    //            dc.painter->fillRect(s,
    //            editor->textStyle(TextStyle::MarkedLine).bg);
...
void EditWidget::paintEvent(QPaintEvent *e) {
  ...
  for (int i = cursor.screenRow(); i >= 0; --i) {
    ...
    if (fileRow < 0)
      painter.fillRect(QRect(0, y - dc.fa - 1, width(), dc.fh + 2),
                       nonTextColor);
    else {
  ...
  if (k->showCursor() && hasFocus()) {
    ...
    QString s =
        k->cursorValid() ? k->line(k->fileRow())[k->fileCol()] : QString("");
