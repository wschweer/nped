...
Für ein sinnvolles Arbeiten mit NPed und AI-Modellen musst du einige MCP-Server installieren und
konfigurieren. Es gibt in diesem Umfeld mittlerweile ein riesiges und weiter schnell wachsendes Ökosystem
so das es kaum möglich ist, hier Empfehlungen zu geben.
Aktuell verwende ich aber nachfolgende MCP's.

#### rust-mcp-filesystem

https://github.com/rust-mcp-stack/rust-mcp-filesystem.git

Der Server startet im r/w Modus und kann nur auf Projektfiles zugreifen:
...
Dies ist der "offizielle" GitHub MCP Server.
...
Dieser Server ermöglicht diverse git Kommandos.
...
https://mcpservers.org/servers/mnhlt/WebSearch-MCP
      ...
      "mcpServers": [
            ...
            "command": "npx",
            "enabled": true,
            "env": "API_URL=\"http://localhost:3001\", MAX_SEARCH_RESULTS=5",
            "id": "websearch",
            "url": ""
            }, ...
...
Für "Frontend" Entwicklung essentiell ist dieser Server, der es u.A. ermöglicht,
Google-Chrome fernzusteuern.
...
Einige Tools sind zusätzlich zu den von den MCP Servern zur Verfügung gestellten Tools
intern implementiert.

Modelle können nur über diese Tools auf dein System zugreifen. In ```Role``` kannst
